...
int vhook_register(vhook_fn fn, uint16_t hz, void *userdata)
{
    ...
    // More precise calculation to avoid truncation
    interval_ticks = (vclock_hz + hz/2) / hz; // Round to nearest instead of truncating
    if (interval_ticks == 0) interval_ticks = 1; // Ensure minimum interval

    vhooks[vhook_count].fn             = fn;
    vhooks[vhook_count].interval_ticks = interval_ticks;
